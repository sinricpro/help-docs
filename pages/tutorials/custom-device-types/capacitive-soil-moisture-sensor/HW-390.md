...
In this part, we'll guide you on building a **Capacitive Soil Moisture Sensor** that can be connected to either an **ESP32, ESP8266, or Raspberry Pi Pico W**. Once it's set up, you'll be able to check the soil moisture level using Alexa, ask Alexa for information about it, and also monitor it through the Sinric Pro app and send a push notification when soil moisture is dry.
...
![Sinric Pro Alexa capacitive soil moisture sensor portal]({{ site.github.url }}/public/img/capacitive-soil-moisture-sensor-portal.png)
